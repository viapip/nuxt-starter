 ...
 */
export type RiskAction = 'BLOCK' | 'DOCUMENT' | 'MITIGATE' | 'MONITOR'
...
export function assessRisk(params: { probability: Probability, impact: Impact, reasoning: string }): RiskAssessment {
  const { impact, probability, reasoning } = params

  const score = calculateRiskScore(probability, impact)
  const action = classifyRiskAction(score)

  return { action, impact, probability, reasoning, score }
}
 ...
 */
export function assessTestScenarios(scenarios: Omit<TestScenario, 'priority' | 'risk'>[]): TestScenario[] {
 ...
 */
export const paymentScenarios: Omit<TestScenario, 'priority'>[] = [
  {
    feature: 'Checkout',
    id: 'PAY-001',
    owner: 'qa-team',
...
${scenarios
  ...
  .map((s) => {
    return `- **[${s.priority}]** ${s.id}: ${s.title} (Score: ${s.risk.score} - ${s.risk.action})`
  })
  .join('\n')}
`.trim()
}
...
export type TrackedRisk = {
  ...
  history: RiskHistory[]
  mitigations: string[]
  status: 'MITIGATED' | 'OPEN' | 'RESOLVED' | 'WAIVED'
}

export class RiskTracker {
  private risks = new Map<string, TrackedRisk>()
  ...
  addRisk(params: {
    ...
    const { changedBy, feature, id, impact, probability, reasoning, title } = params

    const assessment = assessRisk({ impact, probability, reasoning })
  ...
  reassessRisk(params: {
    ...
    const { changedBy, id, impact, probability, reasoning } = params
   ...
   */
  mitigateRisk(params: { id: string, newProbability: Probability, mitigation: string, changedBy: string }): null | TrackedRisk {
    const { changedBy, id, mitigation, newProbability } = params
  ...
  getRisksRequiringAction(): TrackedRisk[] {
    return Array.from(this.risks.values())
      .filter(
        (r) => {
          return r.status === 'OPEN' && (r.currentRisk.action === 'MITIGATE' || r.currentRisk.action === 'BLOCK')
        },
      )
  }
   ...
   */
  generateTrendReport(riskId: string): null | string {
...
${risk.history
  .reverse()
  .map((h) => {
    return `- **${h.timestamp.toISOString()}** by ${h.changedBy}: Score ${h.assessment.score} (${h.assessment.action}) - ${h.reason}`
  })
  .join('\n')}
`.trim()
  }
...
export type GateDecision = 'CONCERNS' | 'FAIL' | 'PASS' | 'WAIVED'
...
export function evaluateGateFromRisks(risks: TrackedRisk[]): GateResult {
  const blockers = risks.filter((r) => {
    return r.currentRisk.action === 'BLOCK' && r.status === 'OPEN'
  })
  const concerns = risks.filter((r) => {
    return r.currentRisk.action === 'MITIGATE' && r.status === 'OPEN'
  })
  const monitored = risks.filter((r) => {
    return r.currentRisk.action === 'MONITOR'
  })
  const documented = risks.filter((r) => {
    return r.currentRisk.action === 'DOCUMENT'
  })
  ...
  const summary = generateGateSummary({ blockers, concerns, decision, documented, monitored })

  return { blockers, concerns, decision, documented, monitored, summary }
}
...
function generateGateSummary(result: Omit<GateResult, 'summary'>): string {
  const { blockers, concerns, decision, documented, monitored } = result
  ...
  if (monitored.length > 0) {
    ...
    for (const r of monitored) {
      lines.push(`- **${r.id}**: ${r.title} (Score: ${r.currentRisk.score})`); continue
    }
  }
