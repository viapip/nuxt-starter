 ...
 */
function extractMagicLink(htmlString: string): null | string {
...
test.describe('Magic Link Authentication', () => {
  test('should authenticate user via magic link', async ({ context, page }) => {
    ...
    await page.getByTestId('email-input')
      .fill(testEmail)
    await page.getByTestId('send-magic-link')
      .click()

    // Assert: Success message
    await expect(page.getByTestId('check-email-message'))
      .toBeVisible()
    await expect(page.getByTestId('check-email-message'))
    ...
    await expect(page.getByTestId('user-menu'))
      .toBeVisible()
    await expect(page.getByTestId('user-email'))
  ...
  test('should prevent reusing magic link', async ({ page }) => {
    ...
    // Sign out
    await page.getByTestId('sign-out')
      .click()
    ...
    await expect(page.getByTestId('error-message'))
      .toBeVisible()
    await expect(page.getByTestId('error-message'))
      .toContainText('link has already been used')
  })
})
```
...
describe('Magic Link Authentication', () => {
  it('should authenticate user via magic link', () => {
    ...
    cy.get('[data-cy="email-input"]')
      .type(testEmail)
    cy.get('[data-cy="send-magic-link"]')
      .click()
    cy.get('[data-cy="check-email-message"]')
      .should('be.visible')
...
export const test = base.extend<EmailAuthFixture>({
  authenticatedUser: async ({ context, page }, use) => {
    ...
    try {
      ...
      const isAuthenticated = await page.getByTestId('user-menu')
...
test.describe('Email Auth Negative Flows', () => {
  test('should reject expired magic link', async ({ page }) => {
    ...
    await expect(page.getByTestId('error-message'))
      .toBeVisible()
    await expect(page.getByTestId('error-message'))
      .toContainText(/link.*expired|expired.*link/i)

    // Assert: Link to request new one
    await expect(page.getByTestId('request-new-link'))
      .toBeVisible()
  ...
  test('should reject invalid magic link token', async ({ page }) => {
    ...
    await expect(page.getByTestId('error-message'))
      .toBeVisible()
    await expect(page.getByTestId('error-message'))
      .toContainText(/invalid.*link|link.*invalid/i)

    // Assert: User not authenticated
    await expect(page.getByTestId('user-menu')).not.toBeVisible()
  })

  test('should reject already-used magic link', async ({ context, page }) => {
    const randomId = Math.floor(Math.random() * 1000000)
    const testEmail = `user-${randomId}@${MAILOSAUR_SERVER_ID}.mailosaur.net`
    ...
    await expect(page.getByTestId('error-message'))
      .toBeVisible()
    await expect(page.getByTestId('error-message'))
      .toContainText(/already.*used|link.*used/i)
  ...
  test('should rate-limit excessive magic link requests', async ({ page }) => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (errorVisible) {
        ...
        await expect(page.getByTestId('rate-limit-error'))
          .toContainText(/too many.*requests|rate.*limit/i)
...
function fillRegistrationForm({ email, fullName, password, userName }) {
  cy.intercept('POST', 'https://cognito-idp*')
    .as('cognito')
  cy.contains('Register')
    .click()
  cy.get('#reg-dialog-form')
    .should('be.visible')
  cy.get('#first-name')
  ...
  return cy
    ...
    .then((code) => {
      ...
      cy.get('#verification-code')
        .type(code, { delay: 0 })
      cy.contains('button', 'Confirm registration')
        .click()
      cy.wait('@cognito')
      cy.contains('You are now registered!')
        .should('be.visible')
      cy.contains('button', /ok/i)
        .click()
...
function register({ email, fullName, password, userName }) {
  fillRegistrationForm({ email, fullName, password, userName })
 ...
 */
Cypress.Commands.add('registerAndSignIn', ({ email, fullName, password, userName }) => {
  return dataSession({
    name: email, // Unique session per email

    // First time: Full registration (form → email → code)
    init: () => {
      return register({ email, fullName, password, userName })
    },
    ...
    recreate: () => {
      return signIn({ password, userName })
    },
