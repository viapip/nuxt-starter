...
test('admin actions', async ({ authOptions, authToken }) => {
...
test.describe.parallel('multi-user tests', () => {
  ...
  test('user 2 actions', async ({ authOptions, authToken }) => {
...
test('ephemeral user test', async ({ context, page }) => {
  ...
  await expect(page.getByTestId('delete-user-btn'))
    .toBeVisible()
...
test('user interaction', async ({ browser }) => {
  ...
  await expect(user2Page.getByText('Hello from user 1'))
    .toBeVisible()
...
const apiAuthProvider: AuthProvider = {
  ...
  getUserIdentifier: (options) => {
    return options.userIdentifier || 'api-user'
  },
  ...
  isTokenExpired: (storageState) => {
    ...
    return Date.now() > Number.parseInt(expiryEntry.value, 10)
  },
  ...
  manageAuthToken: async (request, options) => {
    ...
    const { expiresIn, token } = await response.json()
...
test('should access protected endpoint', async ({ apiRequest, authToken }) => {
...
test('should create resource with auth', async ({ apiRequest, authToken }) => {
  const { body, status } = await apiRequest({
...
const { INTERNAL_SERVICE_URL } = process.env
...
const myCustomProvider: AuthProvider = {
  ...
  getUserIdentifier: (options) => {
    return options.userIdentifier || 'default-user'
  },

  extractToken: (storageState) => {
    // Extract token from your storage format
    return storageState.cookies.find((c) => {
      return c.name === 'auth_token'
    })?.value
  },
  ...
  isTokenExpired: (storageState) => {
    ...
    const expiresAt = storageState.cookies.find((c) => {
      return c.name === 'expires_at'
    })
