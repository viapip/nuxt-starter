...
test('logging demo', async ({ page }) => {
  ...
  await log.debug({ sessionId: 'abc', userId: '123' })
...
```typescript
test('organized with steps', async ({ apiRequest, page }) => {
  ...
  await expect(page.getByText('Updated Name'))
    .toBeVisible()
...
test('should log auth flow', async ({ apiRequest, authToken }) => {
...
```typescript
await log.step({ action: 'create', user: 'test' }) // Shows empty in UI
```
...
await log.debug({ action: 'create', user: 'test' }) // Detailed data
```
...
await log.debug({ itemIds: items.map((i) => {
  return i.id
}) }) // One log entry
```
