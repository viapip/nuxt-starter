...
test.describe('Checkout Flow', () => {
  test('user can complete purchase with saved payment method', async ({ apiRequest, page }) => {
    ...
    await expect(page.getByText('Visa ending in 1234'))
      .toBeVisible() // Saved card
    ...
    await expect(page.getByText('Order Confirmed'))
      .toBeVisible()
    await expect(page.getByText(/Order #\d+/))
      .toBeVisible()
...
test.describe('UserService Integration', () => {
  test('should create user with admin role via API', async ({ request }) => {
    ...
    expect(fetchedUser.role)
      .toBe('admin')
    expect(fetchedUser.permissions)
...
test('calculate discount via UI', async ({ page }) => {
  ...
  await expect(page.getByText('$80'))
    .toBeVisible()
})
