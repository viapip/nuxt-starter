...
export function validateFlag(flag: string): asserts flag is FlagKey {
  if (!Object.values(FLAGS)
    .includes(flag as FlagKey)) {
...
test.describe('Checkout Flow - Feature Flag Variations', () => {
  ...
  test('should use NEW checkout flow when flag is ENABLED', async ({ page, request }) => {
    ...
    await expect(page.getByTestId('checkout-v2-container'))
      .toBeVisible()
    await expect(page.getByTestId('express-payment-options'))
      .toBeVisible()
    await expect(page.getByTestId('saved-addresses-dropdown'))
      .toBeVisible()
  ...
  test('should use LEGACY checkout flow when flag is DISABLED', async ({ page, request }) => {
    ...
    await expect(page.getByTestId('checkout-v1-container'))
      .toBeVisible()
    await expect(page.getByTestId('legacy-payment-form'))
      .toBeVisible()
  ...
  test('should handle flag evaluation errors gracefully', async ({ page, request }) => {
    ...
    await page.route('**/api/feature-flags/evaluate', (route) => {
      return route.fulfill({ body: 'Service Unavailable', status: 500 })
    })
    ...
    expect(consoleErrors)
      .toContain(expect.stringContaining('Feature flag evaluation failed'))
  })
})
```
...
describe('Checkout Flow - Feature Flag Variations', () => {
  ...
  it('should use NEW checkout flow when flag is ENABLED', () => {
    ...
    // Assert
    cy.get('[data-testid="checkout-v2-container"]')
      .should('be.visible')
    cy.get('[data-testid="checkout-v1-container"]')
      .should('not.exist')
  })
...
type FlagVariation = boolean | number | object | string
...
export async function setFlagForUser(flagKey: FlagKey, userId: string, variation: FlagVariation): Promise<void> {
  const response = await playwrightRequest.newContext()
    .then((ctx) => {
      return ctx.post(`${LD_API_BASE}/flags/${flagKey}/targeting`, {
        data: {
          targets: [
            {
              values: [userId],
...
export async function removeFlagTarget(flagKey: FlagKey, userId: string): Promise<void> {
  const response = await playwrightRequest.newContext()
    .then((ctx) => {
      return ctx.delete(`${LD_API_BASE}/flags/${flagKey}/targeting/users/${userId}`, {
        headers: {
          Authorization: LD_SDK_KEY!,
        },
      })
...
export async function setFlagRolloutPercentage(flagKey: FlagKey, percentage: number): Promise<void> {
  ...
  const response = await playwrightRequest.newContext()
    .then((ctx) => {
      return ctx.patch(`${LD_API_BASE}/flags/${flagKey}`, {
        data: {
          rollout: {
            variations: [
...
export const test = base.extend<FeatureFlagFixture>({
  featureFlags: async ({}, use) => {
    const cleanupQueue: { flag: FlagKey, userId: string }[] = []
...
function auditFeatureFlags(): AuditResult {
  ...
  const expiredFlags = getExpiredFlags()
    .map((meta) => {
      return meta.key
    })
  ...
  // Missing metadata
  const missingOwners = allFlags.filter((flag) => {
    return !FLAG_REGISTRY[flag].owner
  })
  const missingDates = allFlags.filter((flag) => {
    return !FLAG_REGISTRY[flag].createdDate
  })
...
function generateReport(audit: AuditResult): string {
  ...
  if (audit.expiredFlags.length > 0) {
    ...
    for (const flag of audit.expiredFlags) {
      const meta = FLAG_REGISTRY[flag]
  ...
  if (audit.flagsNearingExpiry.length > 0) {
    ...
    for (const flag of audit.flagsNearingExpiry) {
      const meta = FLAG_REGISTRY[flag]
      report += `- **${meta.name}** (\`${flag}\`)\n`
      report += `  - Owner: ${meta.owner}\n`
  ...
  if (audit.permanentFlags.length > 0) {
    ...
    for (const flag of audit.permanentFlags) {
      const meta = FLAG_REGISTRY[flag]
