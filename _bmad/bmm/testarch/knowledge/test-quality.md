...
test('user can view dashboard - FLAKY', async ({ page }) => {
  ...
  if (await page.locator('[data-testid="welcome-banner"]')
    .isVisible()) {
  ...
  await expect(page.getByText(randomEmail))
    .toBeVisible() // Will fail randomly
})

// ✅ GOOD: Deterministic test with explicit waits
test('user can view dashboard', async ({ apiRequest, page }) => {
  ...
  const dashboardPromise = page.waitForResponse((resp) => {
    return resp.url()
      .includes('/api/dashboard') && resp.status() === 200
  })
  ...
  await expect(page.getByText(`Welcome, ${user.name}`))
    .toBeVisible()
  await expect(page.getByTestId('dashboard-items'))
    .toHaveCount(dashboard.items.length)
    ...
    cy.wait('@getDashboard')
      .then((interception) => {
        ...
        cy.contains(`Welcome, ${user.name}`)
          .should('be.visible')
        cy.get('[data-cy="dashboard-items"]')
          .should('have.length', dashboard.items.length)
      })
  })
})
```
...
test('admin can create user - POLLUTES STATE', async ({ apiRequest, page }) => {
...
describe('Admin User Management', () => {
  ...
  afterEach(() => {
    ...
    for (const userId of createdUserIds) {
      cy.task('db:delete', { id: userId, table: 'users' })
    }
  ...
  it('should create user', () => {
    ...
    // Track for cleanup
    cy.task('db:findByEmail', newUser.email)
      .then((user: any) => {
...
test('create user via API', async ({ request }) => {
  ...
  expect(createdUser.role)
    .toBe('user')
  expect(createdUser.createdAt)
    .toBeTruthy()
  expect(createdUser.isActive)
    .toBe(true)
...
test.describe('User creation validation', () => {
  const testCases = [
    { expected: 'test@example.com', field: 'email', value: 'test@example.com' },
    { expected: 'Test User', field: 'name', value: 'Test User' },
    { expected: 'admin', field: 'role', value: 'admin' },
    { expected: true, field: 'isActive', value: true },
  ]

  for (const { expected, field, value } of testCases) {
...
test('admin can create user', async ({ adminPage, seedUser }) => {
  ...
  await expect(adminPage.getByText('User created'))
    .toBeVisible()
  await expect(adminPage.getByText(newUser.email))
    .toBeVisible()
...
test('admin can assign permissions', async ({ adminPage, seedUser }) => {
  ...
  await expect(adminPage.getByText('Permissions updated'))
    .toBeVisible()
...
test('admin can update notification preferences', async ({ adminPage, seedUser }) => {
  ...
  await expect(adminPage.getByText('Preferences saved'))
    .toBeVisible()

  // Verify preferences
  const response = await adminPage.request.get(`/api/users/${user.id}/preferences`)
  const prefs = await response.json()
  expect(prefs.emailEnabled)
    .toBe(true)
  expect(prefs.smsEnabled)
    .toBe(false)
  expect(prefs.frequency)
    .toBe('daily')
})
...
test('user completes order', async ({ apiRequest, page }) => {
  ...
  await expect(page.getByText('Order Confirmed'))
    .toBeVisible()
  await expect(page.getByText(`Order #${product.id}`))
    .toBeVisible()
...
describe('Order Flow', () => {
  it('should complete purchase quickly', () => {
    ...
    cy.task('db:seed', { products: [product], users: [user] })
    ...
    // Step 4: Checkout
    cy.visit('/checkout')
    cy.get('[data-cy="credit-card"]')
      .type('4111111111111111')
    cy.get('[data-cy="cvv"]')
      .type('123')
    cy.get('[data-cy="expiry"]')
      .type('12/25')
...
export default async function globalSetup() {
  ...
  const admin = createUser({ emailVerified: true, role: 'admin' })
